...
function efr_solve_unsteady(;
    ...
    filter = nothing,
    relax = nothing,
    cache = efr_ode_method_cache(method, setup, filter),
    # Whether to project the solution after filtering
    ...
    if isadaptive
        while stepper.t < tend
            ...
            stepper = efr_timestep!(method, stepper, Δt; θ, cache,
                                filter, relax, project_after_filter)
